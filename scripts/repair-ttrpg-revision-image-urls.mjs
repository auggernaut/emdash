...
const manifestPath = path.resolve(
	values.manifest || path.join(reportsDir, `revision-image-url-repair-${stamp}.json`),
);
...
const imageById = new Map(imageRows.map((row) => [row.id, row.image_url]));
...
function replacementFor(item, rootImageUrl) {
	return (
		rootImageUrl ||
...
function repairValue(value, rootImageUrl = null) {
	if (Array.isArray(value)) {
		let replacements = 0;
		const repaired = value.map((item) => {
			const result = repairValue(item, null);
	...
	for (const [key, child] of Object.entries(value)) {
		const result = repairValue(child, null);
		repaired[key] = result.value;
		replacements += result.replacements;
	}

	if (typeof value.image_url === "string") {
		const replacement = replacementFor(value, rootImageUrl);
		if (replacement && replacement !== value.image_url) {
			repaired.image_url = replacement;
			replacements += 1;
		}
	}
...
const rows = await runD1Json(
	...
	values.database,
	[
		"SELECT id, entry_id, data",
		"FROM revisions",
		"WHERE collection = 'games'",
		"AND data LIKE '%res.cloudinary.com%'",
...
for (const row of rows) {
	try {
		...
		const rootImageUrl = imageById.get(row.entry_id) || null;
		const result = repairValue(data, rootImageUrl);
		if (result.replacements === 0) continue;
		updates.push({
			id: row.id,
			from: row.data,
			to: JSON.stringify(result.value),
			replacements: result.replacements,
		});
	} catch (error) {
		failed.push({
...
const applyStatements = updates.map((update) =>
	[
		"UPDATE revisions",
		`SET data = ${sqlString(update.to)}`,
		`WHERE id = ${sqlString(update.id)}`,
		`AND data = ${sqlString(update.from)};`,
	].join(" "),
);
const rollbackStatements = updates.map((update) =>
	[
		"UPDATE revisions",
		`SET data = ${sqlString(update.from)}`,
		`WHERE id = ${sqlString(update.id)}`,
		`AND data = ${sqlString(update.to)};`,
	].join(" "),
);
