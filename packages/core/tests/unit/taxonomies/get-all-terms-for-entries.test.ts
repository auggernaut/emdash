...
describe("getAllTermsForEntries", () => {
	...
	it("invalidates the assignment probe cache when terms are set through the handler", async () => {
		await db
		...
		invalidateTermCache();
		expect(await getAllTermsForEntries("post", [post.id])).toEqual(new Map([[post.id, {}]]));

		const result = await handleContentTermsSet(db, "post", post.id, "tag", [tag.id]);
		expect(result.success).toBe(true);

		const hydrated = await getAllTermsForEntries("post", [post.id]);
		expect(hydrated.get(post.id)?.tag?.map((term) => term.slug)).toEqual(["web"]);
	});
